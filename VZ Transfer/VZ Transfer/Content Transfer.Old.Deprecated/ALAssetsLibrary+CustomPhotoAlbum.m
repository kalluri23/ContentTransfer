 ...
 */
- (ALAssetsLibraryWriteImageCompletionBlock)_resultBlockOfAddingToAlbum:(NSArray *)albumName
...
#define ALAssetsLibraryWriteDataDecodingError -3306
- (ALAssetsLibraryWriteImageCompletionBlock)_resultBlockOfAddingToAlbum:(NSArray *)albumName
                                                             completion:(ALAssetsLibraryWriteImageCompletionBlock)completion
                                                                failure:(ALAssetsLibraryAccessFailureBlock)failure
{
  return ^(NSURL *assetURL, NSError *error) {
      ...
      if (assetURL == nil) { // not getting saved properly because of the screen issue
          NSString * message = @"ALAssetsGroup failed to add asset: resolution issue";
          NSError *error = [NSError errorWithDomain:@"LIB_ALAssetsLibrary_CustomPhotoAlbum"
                                               code:ALAssetsLibraryWriteDataDecodingError
      ...
      // save in camera roll success
//      DebugLog(@"camera roll url:%@", [assetURL absoluteString]);
      NSArray *uniquearray = [[NSSet setWithArray:albumName] allObjects];
      int totalAlbumNumber = (int)uniquearray.count;
      if (totalAlbumNumber <= 1) { // only have camera roll, or no album information
          uniquearray = nil;
          if(completion) completion(assetURL, error);
          return;
      }
      ...
      // have custom albums
      __block int albumNumberSavedSuccess = 1;
      for (NSString *name in uniquearray) {
          @autoreleasepool {
              if (![name isEqualToString:@"Camera Roll"]) {
                  ...
                  [self addAssetURL:assetURL
                         ...
                         completion:^(NSURL *assetURL, NSError *error) {
                             ...
                             albumNumberSavedSuccess ++;
                             if (albumNumberSavedSuccess == totalAlbumNumber) {
    ...
    enumerationBlock = ^(ALAssetsGroup *group, BOOL *stop) {
        ...
        if (group == nil && albumWasFound == NO) {
            ...
            if (! [self respondsToSelector:@selector(addAssetsGroupAlbumWithName:resultBlock:failureBlock:)]) {
                DebugLog(@"%s: WARNING: |-addAssetsGroupAlbumWithName:resultBlock:failureBlock:| \
...
#pragma clang diagnostic pop
                    ...
                    if (shouldInvokeSuccessBlockInMainThread) {
                        ...
                        if (createAlbumSucceed) {
                            ...
                            if (error) {
                                DebugLog(@"%s: Error creating album (%@) :  %@",
                   ...
                   completion:(void (^)(NSMutableArray *, NSError *))completion
{
  ...
  ALAssetsLibraryAccessFailureBlock failureBlock = ^(NSError *error) {
    DebugLog(@"%s: %@", __PRETTY_FUNCTION__, [error localizedDescription]);
