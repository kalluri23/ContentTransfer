...
static CTBonjourManager *managerSharedInstance = nil;

@interface CTBonjourManager()
...
@implementation CTBonjourManager
...
+ (instancetype)sharedInstance {
    ...
    dispatch_once(&token, ^{
        managerSharedInstance = [[CTBonjourManager alloc] init];
    });
...
- (void)createServerForController:(nullable id <NSNetServiceDelegate>)controller {
    ...
    self.serviceIdentifier = [NSString stringWithFormat:@"%@/%@",self.serviceDisplayName,[[NSString deviceVID] substringToIndex:8]];
    self.server = [[NSNetService alloc] initWithDomain:LOCAL type:kBonjourType name:self.serviceIdentifier port:0];
//    self.server = [[NSNetService alloc] initWithDomain:@"dgasdgadsg" type:@"gzggadfgag" name:self.serviceIdentifier port:0];
...
- (BOOL)createReconnectServerForController:(nullable id)controller {
    ...
    // create and advertise service our kBonjourType on "local network"
    _serviceIdentifier = [NSString stringWithFormat:@"%@/%@/RC",_serviceDisplayName,[[NSString deviceVID] substringToIndex:8]];
...
- (void)seachingForService:(NSString *)target InListWithHandler:(matchingHandler)matchingHandler
{
    ...
    for (NSNetService *service in self.services) {
        if ([service.name isEqualToString:target]) {
...
// Open streams
- (void)openStreamsForController:(id<NSStreamDelegate>)controller withHandler:(nullable OpenStreamHandler)handler
{
...
- (BOOL)sendStream:(NSData *)message
{
    ...
    if ([self.outputStream hasSpaceAvailable]) {
        ...
        NSLog(@"message sent:%ld", (long)bytesWritten);
        return YES;
    } else {
        return NO;
    }
